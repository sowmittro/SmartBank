...
export default function EmployeeDeposit() {
  const { user } = useAuth();
  ...
  const handleSubmit = () => {
    ...
    addTransaction({
      ...
      approvedBy: user?.id,
      description: description || `Cash deposit by ${user?.name ?? 'employee'}`,
    });
    addNotification({
      accountNumber: c.accountNumber,
      message: `৳${amt.toLocaleString()} has been deposited to your account. New balance: ৳${(c.balance + amt).toLocaleString()}.`,
      type: 'success',
    });
    toast.showSuccess(`৳${amt.toLocaleString()} deposited to ${c.name} successfully.`);
  ...
  return (
    <EmployeeLayout title="Deposit">
      <Box maxWidth={640} mx="auto">
        <Typography variant="h5" fontWeight={700} mb={0.5}>Cash Deposit</Typography>
        <Typography variant="body2" color="text.secondary" mb={3}>Deposit cash into a customer's account.</Typography>
        ...
        <Card sx={{ border: '1px solid', borderColor: 'divider', boxShadow: 'none' }}>
          <CardContent sx={{ p: 3 }}>
            <Stack direction="row" alignItems="center" spacing={1.5} mb={3}>
              ...
              <Typography variant="h6" fontWeight={700}>Deposit Form</Typography>
            </Stack>
            ...
            {customer && (
              <Box sx={{ bgcolor: 'success.50', border: '1px solid', borderColor: 'success.main', borderRadius: 2, p: 2, mb: 2 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Box>
                    <Typography variant="caption" color="success.dark">Customer Verified</Typography>
                    <Typography fontWeight={700} color="success.dark">{customer.name}</Typography>
                    <Typography variant="body2" color="text.secondary">{customer.accountNumber}</Typography>
                  </Box>
                  <Box textAlign="right">
                    <Typography variant="caption" color="text.secondary">Current Balance</Typography>
                    <Typography variant="h6" fontWeight={700} color="success.dark">৳{customer.balance.toLocaleString()}</Typography>
                  </Box>
                </Stack>
              </Box>
            )}
            ...
            <Button variant="contained" color="success" size="large" sx={{ mt: 3 }} onClick={handleSubmit} disabled={!customer}>
              Deposit Cash
            </Button>
          </CardContent>
        </Card>
